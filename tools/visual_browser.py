...
- multi_click: Click MULTIPLE elements at once (for CAPTCHA, checkboxes, etc.)
...
For CAPTCHA (select all squares with X):
1. Find all matching squares, e.g., [12], [15], [18], [21]
2. multi_click label_ids=[12, 15, 18, 21]  ← One action, multiple clicks!
3. click the Verify button"""
        ...
        return {
            ...
            "properties": {
                ...
                "label_ids": {  # === 新增 ===
                    "type": "array",
                    "items": {"type": "integer"},
                    "description": "Multiple label IDs for multi_click (e.g., [1, 5, 8])"
                },
                "text": {
                    "type": "string",
    ...
    def _multi_click(self, label_ids: list = None, **_) -> ToolResult:
        """
        批次點擊多個元素（保持擬人化）
        
        用於 CAPTCHA 等需要選擇多個項目的場景
        """
        if not label_ids:
            return ToolResult(success=False, error="label_ids required (e.g., [1, 5, 8])")
